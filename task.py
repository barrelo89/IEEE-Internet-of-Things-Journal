...
#-----------------------------------------------------Task-------------------------------------------------------------------------------------------------------------------------------------------------
class Tasks():
    #Task의 class를 만든다. Task가 가져야 할 features를 정의하자 먼저.(task_types, 1, 15, 10, expiry)
    def __init__(self, max_value, max_alpha, max_deadline, max_task_size):
        ...
        self.alpha_ = 0.001 + np.around(random.uniform(0, max_alpha), decimals = 2)
        #self.alpha_ = 0.01 + np.around(random.uniform(0.1, max_alpha), decimals = 2)
    ...
    def TaskValueFunction(self, time_unit):
        ...
        for _ in X_axis[:int(self.deadline_/time_unit)+1] :
